...
public class TypeCheck extends VarCheck {
    ...
    public void TypeCheckVarDeclNode(VarDeclNode x) throws SemanticException {
        ...
        for (p = x.vars; p != null; p = p.next) {
            ...
            //checa se o valor inicializado corresponde ao tipo da variavel. Exemplo int num=5 como num � int n�oo pode receber string
...
//    		}
    		...
    		if (q.expr1 instanceof VarNode) {
    			v = Curtable.varFind(q.expr1.position.image);
    ...
    //
    		...
    		t2 = TypeCheckExpreNode((ExpreNode) q.expr1); 
    //
    ...
    public void TypeCheckMethodDeclNode(MethodDeclNode x)
        ...
        while (p != null) {
            ...
            g = Curtable.varFind(u.position.image);
            //System.out.println("Valor de g" + g);
            if ((u.expr1!=null) && (g!=null)) {
               
            		id_valor_atribuido=u.expr1.position.kind;           
            		tipo_variavel=g.type.name;
            	
        	if (id_valor_atribuido==FunConstants.float_literal && tipo_variavel=="float") {
        		 System.out.println("Valor local de float ok");
        	}	else if (id_valor_atribuido==FunConstants.boolean_constant && tipo_variavel=="boolean") {
        		System.out.println("Valor local de boolean ok");
        	} else if (id_valor_atribuido==FunConstants.char_const && tipo_variavel=="char") {
        		System.out.println("Valor local de char ok");
        	} else if (id_valor_atribuido==FunConstants.int_literal && tipo_variavel=="int") {
        		System.out.println("Valor local de int ok");
        	} else if (id_valor_atribuido==FunConstants.string_constant && tipo_variavel=="string") {
        		System.out.println("Valor local de string ok");
        	}	else {
        		throw new SemanticException(u.position,
                        "Tipo de variavel: "+tipo_variavel+ " n�o corresponde ao tipo do valor inicializado ");
    ...
    public void TypeCheckAtribNode(AtribNode x) throws SemanticException {
        ...
        if (!(isSubClass(t2.ty, t1.ty) || isSubClass(t1.ty, t2.ty))) {
            throw new SemanticException(x.position,
                "Tipos incompativeis para a igualdade ");
        }
    }
        ...
        try {
            ...
            if (((t.ty != INT_TYPE) && (t.ty != BOOLEAN_TYPE)) || (t.dim != 0)) {
                throw new SemanticException(x.expr.position,
                    "Express�o inteira ou booleana experada");
            }
        } catch (SemanticException e) {
            System.out.println(e.getMessage());
            foundSemanticError++;
    ...
    public type TypeCheckRelationalNode(RelationalNode x)
        ...
        if (((t1.ty == INT_TYPE) || (t1.ty == FLOAT_TYPE  )) && ((t2.ty == INT_TYPE)||(t2.ty == FLOAT_TYPE))) {
            return new type(BOOLEAN_TYPE, 0);
        }
        
        if ((t1.ty == t2.ty) && (op==FunConstants.EQ || op==FunConstants.NEQ)) {
        ...
        throw new SemanticException(x.position,
            "Tipos inv�lidos para " + x.position.image);
    }
    ...
    public type TypeCheckAddNode(AddNode x) throws SemanticException {
        ...
        type t2;
        int op; // opera��o
        int i;//contador inteiros
        int j;//contador strings
        int c;//contador char
        int b;//contador boleanos
        int f;//contador float
        ...
        // um inteiro e um char s� pode somar
        if ((op == FunConstants.PLUS) && ((i + c) == 2)) {
            return new type(STRING_TYPE, 0);
        }
        
     // um inteiro e um booleano s� pode somar
        if ((op == FunConstants.PLUS) && ((i + b) == 2)) {
            return new type(STRING_TYPE, 0);
        }
    ...
    public type TypeCheckMultNode(MultNode x) throws SemanticException {
        ...
        // s� int's e floats s�o aceitos
        if (((t1.ty != INT_TYPE) && (t1.ty != FLOAT_TYPE) ) || ((t2.ty != FLOAT_TYPE) && (t2.ty != INT_TYPE))) {
            throw new SemanticException(x.position,
                "Tipos Inv�lidos " + x.position.image);
        }
 ...
 // ---------------------- OR|XOR --------------------
    public type TypeCheckOrNode(OrNode x) throws SemanticException {
        ...
        // s� dois boolean s�o aceitos
        if (((t1.ty != BOOLEAN_TYPE) && (t1.ty != INT_TYPE)) || ( (t2.ty != BOOLEAN_TYPE)&& (t2.ty != INT_TYPE))) {
            throw new SemanticException(x.position,
                "Tipo invalido " + x.position.image);
        }
 ...
 // -------------------------- Constante inteira ----------------------
    public type TypeCheckIntNode(IntNode x) throws SemanticException {
        int k;
    ...
    // ------------------------ Constante string ----------------------------
    public type TypeCheckStringNode(StringNode x) {
        if (x == null) {
            return null;
        }
    ...
    // ------------------------ Constante char ----------------------------
    public type TypeCheckCharNode(CharNode x) {
        if (x == null) {
            return null;
        }
    ...
    // ------------------------ Constante float ----------------------------
    public type TypeCheckFloatNode(FloatNode x) {
        if (x == null) {
            return null;
        }
    ...
    // ------------------------ Constante boolena ----------------------------
    public type TypeCheckBooleanNode(BooleanNode x) {
        if (x == null) {
            return null;
        }
    ...
    // ------------------------------ Constante null --------------------------
    public type TypeCheckNullNode(NullNode x) {
        if (x == null) {
            return null;
        }
    ...
    public type TypeCheckCallNode(CallNode x) throws SemanticException {
        ...
        //System.out.println(x.args.position.next.image);
 ...
 // ---------------------------- Chamada de metodo ------------------------
    ...
    public type TypeCheckExpreNode(ExpreNode x) throws SemanticException {
        if (x instanceof NewObjectNode) {
            ...
            return TypeCheckIntNode((IntNode) x);
        } else if (x instanceof BooleanNode) {
            return TypeCheckBooleanNode((BooleanNode) x);
        } else if (x instanceof CharNode) {
            return TypeCheckCharNode((CharNode) x);
        } else if (x instanceof FloatNode) {
            return TypeCheckFloatNode((FloatNode) x);
        } else if (x instanceof StringNode) {
            return TypeCheckStringNode((StringNode) x);
        } else if (x instanceof NullNode) {
            return TypeCheckNullNode((NullNode) x);
